...
// <<< NOVO: Importando Formulario para tipagem
...
// <<< NOVO: Importando ícones
import { Plus, Loader2, MoreVertical, Edit, Trash2 } from "lucide-react";
import { useToast } from "@/hooks/use-toast";


// (O componente CreateQuestionModal permanece o mesmo, então foi omitido por brevidade)
...
// É bom separá-lo para manter o código limpo
const EditFormModal = ({ form, isOpen, onOpenChange }: { form: Formulario | null, isOpen: boolean, onOpenChange: (open: boolean) => void }) => {
    ...
    const handleUpdate = async () => {
        if (!form) return;
        await updateForm(form.id, { titulo, descricao, idsPerguntas: selectedQuestionIds });
        onOpenChange(false);
    };
        ...
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-3xl">
                <DialogHeader>
                    ...
                    <DialogDescription>Altere o título, descrição e as perguntas do seu formulário.</DialogDescription>
                </DialogHeader>
                 <div className="grid grid-cols-2 gap-8 py-4">
                    <div className="space-y-4">
                        <h3 className="font-semibold">Detalhes do Formulário</h3>
                        <div><Label htmlFor="edit-form-title">Título</Label><Input id="edit-form-title" value={titulo} onChange={(e) => setTitulo(e.target.value)} /></div>
                        <div><Label htmlFor="edit-form-desc">Descrição</Label><Input id="edit-form-desc" value={descricao} onChange={(e) => setDescricao(e.target.value)} /></div>
                    </div>
                    <div className="space-y-4">
                        ...
                        <div className="space-y-2 max-h-60 overflow-y-auto border p-2 rounded-md">
                            {perguntas.map(p => (
                                <div key={p.id} className="flex items-center space-x-2">
                                    ...
                                    />
                                    <label htmlFor={`edit-q-${p.id}`} className="text-sm">{p.texto}</label>
                                </div>
                            ))}
                        </div>
                    </div>
...
export const FormsPage = () => {
    // <<< NOVO: obtendo as novas funções do contexto
  const { formularios, perguntas, addForm, deleteForm, getFormById, loading } = useForm();
    ...
    // <<< NOVO: Estado de loading para o botão de editar específico >>>
    ...
    const handleCreateForm = async () => {
        ...
        // <<< NOVO: Função para lidar com o clique no botão de editar >>>
    const handleEditClick = async (formId: string) => {
        setEditingId(formId); // Ativa o estado de loading para este item
        try {
            const formData = await getFormById(formId);
            if (formData) {
                setFormToEdit(formData); // Popula o modal com os dados mais recentes
            }
        } finally {
            setEditingId(null); // Desativa o estado de loading
        }
    };
 ...
 <Card>
                                            ...
                                            <DropdownMenu>
                                                <DropdownMenuTrigger asChild>
                                                    <Button variant="ghost" size="icon">
                                                        {/* <<< NOVO: Mostra o loader se estiver buscando dados para este form >>> */}
                                                        {editingId === form.id ? (
                                                ...
                                                <DropdownMenuContent align="end">
                                                    {/* <<< ATUALIZADO: Chama handleEditClick ao selecionar >>> */}
                                                    <DropdownMenuItem onSelect={() => handleEditClick(form.id)} disabled={!!editingId}>
                                                        <Edit className="w-4 h-4 mr-2" /> Editar
                                                    </DropdownMenuItem>
                                                    <AlertDialogTrigger asChild>
                                                        <DropdownMenuItem onSelect={(e) => e.preventDefault()} className="text-red-600" disabled={!!editingId}>
                                            ...
                                            <AlertDialogContent>
                                                <AlertDialogHeader>
                                                    ...
                                                    <AlertDialogDescription>
                                                        Esta ação não pode ser desfeita. Isso excluirá permanentemente o formulário "{form.titulo}".
                                                    </AlertDialogDescription>
                                                </AlertDialogHeader>
                                                <AlertDialogFooter>
            ...
            {/* <<< NOVO: Modal de Edição renderizado aqui */}
            <EditFormModal 
                isOpen={!!formToEdit}
